...
- 1970년 생일 : 타임스탬프가 1970년 1월 1일 부터 현재시간 까지 초단위로 경과한 값
- 1970년 1월 1일부터 16억 2천...초가 흐른시간이 현재시간.
- 부트스트랩용어 : grid(12칸) 시스템, modal, xs, sm, md, lg, bs-부트스트랩, fa-폰트어썸
- 팝업창 : 모달창(nodal - 필수창 : 작업 후 다음으로 이동가능), 모달리스창(modalless)
- 폰트 어썸(아이콘 웹폰트) : 아이콘을 확대해도 깨지지 않음.(벡터방식의 아이콘)
- 포토샵 디자인으로 아이콘을 만들면 확대 시 깨짐(비트맵 = 스칼라 방식의 아이콘)
- 부트스트랩 레이아웃 : 화면을 12개의 칼럼으로 분리해서 크기를 반응형으로 만듭니다.
- row(가로), cols(세로), col-md-12(화면가로 크기를 12로 지정 =100%, 조건은 md에서는 100% )
- 반응형의 화면크기 : xs엑스트스몰(~750px), sm스몰(750px~), md미디엄(970px~), lg라지(1170px~)
...
- jsp(1세대 2000년) -> 서블렛(2세대 serverlet 게시판) -> 스트러츠(3세대) -> 스프링(4세대 2013년대, 프레임워크)
